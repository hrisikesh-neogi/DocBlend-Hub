...
class PDFSummarizerPipeline(BasePipeline):
    video_url = None
    model = OpenAI()
    ...
    def get_map_reduce_chain(model: OpenAI):
        ...
        try:
            ...
            map_template = """The following is a set of documents containing the contents of a pdf.
            {docs}
            Based on this list of docs, please generate the summary of the pdf.
            Helpful Answer:"""
            ...
            reduce_template = """The following is set of summaries of a pdf's content:
            {doc_summaries}
            Take these and distill it into a final, consolidated summary of that particular pdf document. 
            Helpful Answer:"""
    ...
    def summarize(self):
        try:
            ...
            chain = PDFSummarizerPipeline.get_map_reduce_chain(model=self.model)
    ...
    def summarize_pdf(self, uploaded_doc: UploadedFile):
        if uploaded_doc:
            ...
            chain = PDFSummarizerPipeline.get_map_reduce_chain(model=OpenAI())
            output = chain(chunks)
            return output
